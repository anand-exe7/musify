...
import { chargeBase, grossTotal, balanceDue, statusMeta, type RepairTicket } from "@/lib/store/repair";
import { BUSINESS, branchInfo } from "@/lib/data/business";
import { amountInWords, calculateGST, formatINR } from "@/lib/utils";

/**
 * Print stylesheet shared by the admin and customer-facing service invoices —
 * isolates `#invoice-sheet` so Print / Save-PDF captures only the invoice.
 */
export const SERVICE_INVOICE_PRINT_CSS = `
...
/**
 * The service (repair) invoice sheet — pure presentation, driven entirely by the
 * ticket. Rendered by both the admin invoice page (`/admin/service/[id]/invoice`)
 * and the customer-facing share page (`/invoice/service/[id]`), so the number the
 * customer sees always matches what staff issued.
 */
export function ServiceInvoiceSheet({ ticket: t }: { ticket: RepairTicket }) {
  ...
  const dateStr = fmtDate(t.completedAt || t.updatedAt || new Date().toISOString());
